#!/usr/bin/env python3
# gta_hexviewer_extended.py
# Extended with: context actions (insert bytes, add/remove reloc entries, import bin, export selection),
# paste hex into selection, EditChunk dialog, save settings (Ctrl+S) and save file (Ctrl+Shift+S).
...
class VirtualHexView(QtWidgets.QAbstractScrollArea):
    ...
    def contextMenuEvent(self, ev):
        ...
        if off is not None:
            ...
            # New actions requested by user:
            menu.addSeparator()
            act_insert = menu.addAction("Shift bytes (insert N bytes at this offset)")
            def insert_bytes_action():
                n, ok = QtWidgets.QInputDialog.getInt(self, "Insert bytes", "Number of bytes to insert:", 1, 1, 1024*1024, 1)
                if ok:
                    self.parent().insert_bytes_at(off, n)
            act_insert.triggered.connect(insert_bytes_action)

            act_add_reloc = menu.addAction("Add to reloc table (if exists)")
            def add_reloc_action():
                self.parent().add_reloc_at(off)
            act_add_reloc.triggered.connect(add_reloc_action)

            act_remove_reloc = menu.addAction("Remove from reloc table (if present)")
            def rem_reloc_action():
                self.parent().remove_reloc_at(off)
            act_remove_reloc.triggered.connect(rem_reloc_action)

            act_import_bin = menu.addAction("Import BIN here (insert/replace)")
            def import_bin_action():
                self.parent().import_bin_at(off)
            act_import_bin.triggered.connect(import_bin_action)

            act_export_sel = menu.addAction("Export selection as .bin")
            def export_sel_action():
                self.parent().export_selection_bin()
            act_export_sel.triggered.connect(export_sel_action)

        else:
            # menu on empty area
            act_paste_global = menu.addAction("Paste hex (from clipboard) into file at cursor)")
            act_paste_global.triggered.connect(lambda: self.parent().paste_hex_into_file())
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("GTA DTZ Hex Viewer (extended)")
        ...
        # EditChunk button
        tb.addWidget(QtWidgets.QPushButton("EditChunk", clicked=self.open_edit_chunk))
        # settings
        ...
        # Ctrl+S now saves settings (json) only
        QtWidgets.QShortcut(QtGui.QKeySequence("Ctrl+S"), self, activated=self.save_settings_only)
        # Save file with Ctrl+Shift+S
        QtWidgets.QShortcut(QtGui.QKeySequence("Ctrl+Shift+S"), self, activated=self.save_file)
        QtWidgets.QShortcut(QtGui.QKeySequence("Ctrl+Shift+C"), self, activated=self.copy_address)
        QtWidgets.QShortcut(QtGui.QKeySequence("Ctrl+V"), self, activated=self.paste_hex_from_clipboard)
    ...
    # ---------- paste from clipboard (hex) ----------
    def paste_hex_from_clipboard(self):
        txt = QtWidgets.QApplication.clipboard().text().strip()
        if not txt: return
        # accept formats: "AA BB CC" or "0xAABBCC" or continuous "AABBCC"
        hexchars = re.findall(r'[0-9A-Fa-f]+', txt)
        if not hexchars: return
        # if single long group, split every two chars
        if len(hexchars) == 1:
            s = hexchars[0]
            if len(s) % 2 != 0:
                QtWidgets.QMessageBox.warning(self, 'Paste hex', 'Hex string must have even length'); return
            data = bytes(int(s[i:i+2],16) for i in range(0,len(s),2))
        else:
            # groups like "AA BB CC" or "0xAA 0xBB"
            data = bytes(int(h,16) for h in hexchars)
        self._apply_paste_bytes(data)

    def _apply_paste_bytes(self, data: bytes):
        if self.view.sel_start is not None and self.view.sel_end is not None:
            s = min(self.view.sel_start, self.view.sel_end); e = max(self.view.sel_start, self.view.sel_end)
            maxlen = e - s + 1
            to_write = data[:maxlen]
            for i,b in enumerate(to_write): self.view.data[s+i] = b
            self.view.dirty = True; self.view.viewport().update(); self.status.showMessage(f'Pasted {len(to_write)} bytes into selection',2000)
        else:
            off = self.view.caret_offset if self.view.caret_offset is not None else (self.view.hovered if self.view.hovered>=0 else None)
            if off is None: QtWidgets.QMessageBox.warning(self, 'Paste', 'No caret/hover to paste into'); return
            # insert bytes at off
            self.insert_bytes_at(off, 0, replace=False, data=data)
    ...
    # ---------- save settings only (Ctrl+S) ----------
    def save_settings_only(self):
        if not hasattr(self, 'current_file') or not self.current_file:
            QtWidgets.QMessageBox.information(self, 'Save settings', 'No file open to associate settings with.')
            return
        self.save_settings_for_file(self.current_file)

    # ---------- edit chunk header / reloc table ----------
    def open_edit_chunk(self):
        if not self.view.chunk_header:
            QtWidgets.QMessageBox.information(self, 'EditChunk', 'No chunk header parsed in current file.')
            return
        ch = self.view.chunk_header
        dlg = QtWidgets.QDialog(self); dlg.setWindowTitle('Edit Chunk Header')
        layout = QtWidgets.QFormLayout(dlg)
        le_ident = QtWidgets.QLineEdit(str(ch['ident']))
        le_ident_raw = QtWidgets.QLineEdit(hex(ch['ident_raw']))
        le_shrink = QtWidgets.QLineEdit(str(ch['shrink']))
        le_fileEnd = QtWidgets.QLineEdit(hex(ch['fileEnd']))
        le_dataEnd = QtWidgets.QLineEdit(hex(ch['dataEnd']))
        le_relocTab = QtWidgets.QLineEdit(hex(ch['relocTab']))
        le_numRelocs = QtWidgets.QLineEdit(str(ch['numRelocs']))
        layout.addRow('ident', le_ident); layout.addRow('ident_raw', le_ident_raw); layout.addRow('shrink', le_shrink)
        layout.addRow('fileEnd', le_fileEnd); layout.addRow('dataEnd', le_dataEnd); layout.addRow('relocTab', le_relocTab); layout.addRow('numRelocs', le_numRelocs)
        btns = QtWidgets.QDialogButtonBox(QtWidgets.QDialogButtonBox.Ok | QtWidgets.QDialogButtonBox.Cancel)
        layout.addRow(btns)
        btns.accepted.connect(dlg.accept); btns.rejected.connect(dlg.reject)
        if dlg.exec_() == QtWidgets.QDialog.Accepted:
            try:
                # simple parsing (allow hex)
                def p(x):
                    t = x.text().strip();
                    if t.lower().startswith('0x'): return int(t,16)
                    return int(t,10)
                ch['ident_raw'] = p(le_ident_raw)
                ch['shrink'] = p(le_shrink)
                ch['fileEnd'] = p(le_fileEnd)
                ch['dataEnd'] = p(le_dataEnd)
                ch['relocTab'] = p(le_relocTab)
                ch['numRelocs'] = p(le_numRelocs)
                # write back header to data
                self._write_chunk_header_to_data()
                QtWidgets.QMessageBox.information(self, 'EditChunk', 'Header written into data. You may need to Edit reloc table separately.')
                self.view.load_bytes(bytes(self.view.data), header_size=self.view.header_size, absolute_offset=self.view.absolute_offset)
            except Exception as e:
                QtWidgets.QMessageBox.warning(self, 'EditChunk', f'Failed: {e}')

    def _write_chunk_header_to_data(self):
        ch = self.view.chunk_header
        if not ch: return
        try:
            struct.pack_into('<7I2H', self.view.data, 0, ch['ident_raw'], ch['shrink'], ch['fileEnd'], ch['dataEnd'], ch['relocTab'], ch['numRelocs'], ch['globalTab'], ch['numClasses'], ch['numFuncs'])
        except Exception as e:
            QtWidgets.QMessageBox.warning(self, 'WriteHeader', f'Failed to write header: {e}')

    # ---------- insert / shift bytes ----------
    def insert_bytes_at(self, offset:int, count:int, replace:bool=False, data:bytes=None):
        # If data is provided, insert that data; otherwise insert zero bytes of length count.
        if offset is None or offset < 0 or offset > len(self.view.data):
            QtWidgets.QMessageBox.warning(self, 'Insert bytes', 'Invalid offset'); return
        if data is not None:
            to_ins = bytearray(data)
        else:
            to_ins = bytearray(b'\x00' * count)
        if replace and count>0:
            # replace 'count' bytes starting at offset
            end = min(offset+count, len(self.view.data))
            self.view.data[offset:end] = to_ins[:end-offset]
        else:
            self.view.data[offset:offset] = to_ins
        # update reloc entries and header offsets
        self._adjust_relocs_after_insert(offset, len(to_ins))
        self.view.dirty = True
        self.view._parse_chunk_header()
        self.view._compute_pad_offsets()
        self.view.update_scrollbars(); self.view.viewport().update(); self.status.showMessage(f'Inserted {len(to_ins)} bytes at 0x{offset:X}',2000)

    def _adjust_relocs_after_insert(self, offset:int, delta:int):
        # shift reloc entries >= offset by delta
        new_entries = []
        for v in self.view.reloc_entries:
            new_entries.append(v + (delta if v >= offset else 0))
        self.view.reloc_entries = sorted(set(new_entries))
        # update chunk header fields that represent offsets if present
        ch = self.view.chunk_header
        if ch:
            for key in ('relocTab','fileEnd','dataEnd','globalTab'):
                if ch.get(key,0) >= offset:
                    ch[key] += delta
            # write back header
            try:
                struct.pack_into('<7I2H', self.view.data, 0, ch['ident_raw'], ch['shrink'], ch['fileEnd'], ch['dataEnd'], ch['relocTab'], ch['numRelocs'], ch['globalTab'], ch['numClasses'], ch['numFuncs'])
            except Exception as e:
                print('Failed write header after insert', e)

    # ---------- add/remove reloc helpers ----------
    def add_reloc_at(self, offset:int):
        if not self.view.chunk_header:
            QtWidgets.QMessageBox.information(self, 'Add reloc', 'No reloc table parsed or header not present.')
            return
        if offset in self.view.reloc_entries:
            QtWidgets.QMessageBox.information(self, 'Add reloc', f'Offset 0x{offset:X} already in reloc table.')
            return
        self.view.reloc_entries.append(offset); self.view.reloc_entries = sorted(set(self.view.reloc_entries))
        self.view.pointer_fields = set(self.view.reloc_entries)
        self.view.pointer_byte_offsets.clear();
        for p in self.view.pointer_fields:
            for i in range(4): self.view.pointer_byte_offsets.add(p+i)
        # try to write reloc table into data (will expand if necessary)
        self._write_reloc_table_to_data()
        self.view.dirty=True; self.view.viewport().update(); self.status.showMessage(f'Added reloc entry 0x{offset:X}',2000)

    def remove_reloc_at(self, offset:int):
        if offset not in self.view.reloc_entries:
            QtWidgets.QMessageBox.information(self, 'Remove reloc', f'Offset 0x{offset:X} not present in reloc table.')
            return
        self.view.reloc_entries = [v for v in self.view.reloc_entries if v != offset]
        self.view.pointer_fields = set(self.view.reloc_entries)
        self.view.pointer_byte_offsets.clear();
        for p in self.view.pointer_fields:
            for i in range(4): self.view.pointer_byte_offsets.add(p+i)
        self._write_reloc_table_to_data()
        self.view.dirty=True; self.view.viewport().update(); self.status.showMessage(f'Removed reloc entry 0x{offset:X}',2000)

    def _write_reloc_table_to_data(self):
        ch = self.view.chunk_header
        if not ch:
            QtWidgets.QMessageBox.information(self, 'Write reloc', 'No chunk header info to place reloc table.')
            return
        relocTab = ch['relocTab']
        num = len(self.view.reloc_entries)
        needed = num * 4
        if relocTab + needed > len(self.view.data):
            # expand by inserting needed bytes at relocTab
            self.insert_bytes_at(relocTab, relocTab + needed - len(self.view.data))
        # write entries
        for i,v in enumerate(self.view.reloc_entries):
            struct.pack_into('<I', self.view.data, relocTab + i*4, v)
        ch['numRelocs'] = num
        # update header numRelocs
        try:
            struct.pack_into('<7I2H', self.view.data, 0, ch['ident_raw'], ch['shrink'], ch['fileEnd'], ch['dataEnd'], ch['relocTab'], ch['numRelocs'], ch['globalTab'], ch['numClasses'], ch['numFuncs'])
        except Exception as e:
            print('Failed to write header after reloc write', e)

    # ---------- import bin at offset ----------
    def import_bin_at(self, offset:int):
        # open file, ask whether to insert (shift) or replace (no shift)
        p, _ = QtWidgets.QFileDialog.getOpenFileName(self, 'Import binary', '.', 'BIN files (*.bin);;All files (*)')
        if not p: return
        try:
            with open(p, 'rb') as f: data = f.read()
        except Exception as e:
            QtWidgets.QMessageBox.critical(self, 'Import', f'Failed read: {e}'); return
        # ask for mode and size override
        dlg = QtWidgets.QDialog(self); dlg.setWindowTitle('Import options'); form = QtWidgets.QFormLayout(dlg)
        spin = QtWidgets.QSpinBox(); spin.setRange(0, 10_000_000); spin.setValue(len(data))
        chk_shift = QtWidgets.QCheckBox('Shift data (insert)'); chk_shift.setChecked(True)
        form.addRow('Load size:', spin); form.addRow(chk_shift)
        btns = QtWidgets.QDialogButtonBox(QtWidgets.QDialogButtonBox.Ok | QtWidgets.QDialogButtonBox.Cancel); form.addRow(btns)
        btns.accepted.connect(dlg.accept); btns.rejected.connect(dlg.reject)
        if dlg.exec_() != QtWidgets.QDialog.Accepted: return
        size = spin.value(); shift = chk_shift.isChecked()
        to_write = data[:size]
        if shift:
            self.insert_bytes_at(offset, 0, replace=False, data=to_write)
        else:
            # replace up to size
            self.insert_bytes_at(offset, size, replace=True, data=to_write)
        self.status.showMessage(f'Imported {len(to_write)} bytes from {os.path.basename(p)}',3000)

    # ---------- export selection to .bin ----------
    def export_selection_bin(self):
        if self.view.sel_start is None or self.view.sel_end is None:
            QtWidgets.QMessageBox.information(self, 'Export selection', 'No selection')
            return
        s = min(self.view.sel_start, self.view.sel_end); e = max(self.view.sel_start, self.view.sel_end)
        b = bytes(self.view.data[s:e+1])
        p, _ = QtWidgets.QFileDialog.getSaveFileName(self, 'Export selection as', '.', 'BIN files (*.bin);;All files (*)')
        if not p: return
        try:
            with open(p, 'wb') as f: f.write(b)
            self.status.showMessage(f'Exported {len(b)} bytes to {p}', 3000)
        except Exception as e:
            QtWidgets.QMessageBox.critical(self, 'Export', str(e))

    def toggle_compact(self, on:bool):
        # Toggle compact view from toolbar button (keeps parity with original behavior)
        try:
            self.view.compact_view = bool(on)
            self.view.update_scrollbars()
            self.view.viewport().update()
        except Exception as e:
            print('toggle_compact error:', e)
